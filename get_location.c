...
char *get_location(char *command)
{
	...
	if (path)
	{
		...
		while (path_token != NULL)
		{
			...
			/*Allocates memory for storing the command name together with the directory name*/
			file_path = malloc(command_length + directory_length + 2);
			/*NB: we added 2 for the slash and null character,we will introduce it*/

			/*lets test if this path actually exists and return it if it does, otherwise try the next directory*/
			if (stat(file_path, &buffer) == 0)
			{
				/*return value pf 0 means success implying that the file_path is valid*/
